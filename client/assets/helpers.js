...
function createPosts(data) {
  ...
  for (let i = data.length - 1; i >= 0; i--) {
    ...
    reactions.append(button); // Collapsible button created earlier
    newArticle.append(reactions);

    // A DROPDOWN BOX DISPLAYING ALL COMMENTS MADE TO THE POST
    ...
    // BUTTON SHAPED LIKE + TO ADD A NEW COMMENT:
    const addCommentBtn = document.createElement("button");
    addCommentBtn.classList.add("add-comment-button"); // Corrected so it works with CSS
